            ...
            <CardContent>
              ...
              {s.status === "completed" && s.finalMetrics && (
                <div className="mt-6 space-y-4">
                  ...
                  {Object.entries(s.finalMetrics.per_label_metrics).some(
                    ([, m]) => m.errors.length > 0,
                  ) && (
                    <div>
                      <div className="text-sm font-semibold text-gray-700">Error details by step</div>
                      <div className="mt-2 space-y-3">
                        {Object.entries(s.finalMetrics.per_label_metrics)
                          .filter(([, m]) => m.errors.length > 0)
                          .map(([label, m]) => (
                            <div key={label}>
                              <p className="mb-1.5 font-mono text-xs text-red-700">{label}</p>
                              <ul className="space-y-1.5">
                                {m.errors.map((err, i) => (
                                  <li
                                    key={i}
                                    className="flex items-start justify-between gap-3 rounded-md border border-red-200 bg-red-50 px-3 py-2"
                                  >
                                    <div>
